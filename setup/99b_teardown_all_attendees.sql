/* ============================================================================
   Teardown — all attendees at once

   Discovers every TRANSFORMED_* schema and every PARTNER_* role/user, so it
   clears a whole room in one run. Uses Snowflake Scripting.

   For a single person, or if you want to read every DROP before it runs, use
   99_teardown.sql instead - it is plain statements with no blocks.
